...
facemaps = {'lh-medialorbitofrontal' , 'rh-medialorbitofrontal'}; %'rh_pFus_Faces' 'lh_pFus_Faces'
% wordmaps = {'rh_OTS1_WordsNumbers' 'lh_OTS1_WordsNumbers'};

for i = 1:length(sessions) %we have 1 model fit to averaged 7 runs
 ...
 %  cd(fullfile(sessionDir,sessions{i}))
   ...
   for d = 1:numel(dataTYPES)
       if strmatch('Averages', dataTYPES(1,d).name)
           nomoFlag = true;
       end
   end
   ...
   if nomoFlag
       ...
       rmPath = fullfile('Gray','Averages','ONEGAUSSIAN_wholebrain-fFit-fFit-fFit.mat');
