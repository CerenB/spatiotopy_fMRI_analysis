function Results = calculate_sigma
...
% PC at LLN
path = '/Cerens_files/fMRI/Processed/Spatio_pRF/SC/AlSa/Gray/Averages';
%Ceren's laptop
...
for ivox=1:length(RF)
    ...
    %calculate the confidence interval - not good!
    % [mu ul ll] = circ_mean(alpha, w);
    
    %variance - doesn't make sense now because it's not weighted theta
    %S = circ_var(alpha);
    ...
    %or for big kappa values - use log
    ...
    try
        ...
        %% another way of creating the data, without w but it leads to similar kappa and thetahat
        % still it could be better in the longrun.
        ...
        [all_thetahat_w, all_kappa_w] = circ_vmpar(all_theta_w); %thetahat and kappa
        [mu_all, ul_all, ll_all] = circ_mean(all_theta_w); %CI
        %     rw_all = circ_r(all_theta_w); %radius
        %     Sw_all = circ_var(all_theta_w); % variance
        %     [b_all, b0_all] = circ_skewness(all_theta_w); %skewness
        %     [k_all k0_all] = circ_kurtosis(all_theta_w); %kurtosis % is not zero, means it's not really mises function shaped
        %
        %     % to plot VM function with the thetahat & kappa already created
        %     alpha = linspace(0, 7*pi/4, 101)';
        %     alpha = alpha(1:end-1);
        %
        %     % evaluate pdf
        ...
        %     % this gives exactly the same parameters: A_w == A, p == p_w
        %
        
        % notes: compute across all the voxels, takes too long
        % instead think about using stim_RF(:,ivox) as weight (round them up)-
        % maybe normalize them as well, thinking about VM function indicating
        % normal distribution, so maybe normalisation could help. Though I suspect,
        % it's already normalised because of locate_RF produced stim_RF with
        % normalisation.
    %% space for catch/try
        catch ME
    ...
    %1300 theta weight is distributed
    Results.mu_w_all(ivox) = all_thetahat_w;
    Results.mu_ulw_all(ivox) = ul_all;
    Results.mu_llw_all(ivox) = ll_all;
    Results.kappa_w_all(ivox) = all_kappa_w;
    
    
    Results.r(ivox) = r;
    Results.rw(ivox) = rw;
    Results.Sw(ivox) = Sw;
    Results.rw_all(ivox) = rw_all;
    Results.Sw_all(ivox) = Sw_all;
    
    Results.b(ivox) = b;
    Results.b0(ivox) = b0;
    Results.k(ivox) = k;
    Results.k0(ivox) = k0;
    Results.b_all(ivox) = b_all;
    Results.b0_all(ivox) = b0_all;
    Results.k_all(ivox) = k_all;
    Results.k0_all(ivox) = k0_all;
    
    Results.pUniform(ivox) = p_rayleigh;
    Results.pRao(ivox) = p_rao;
    %
    %     Results.coeff(ivox) = A;
    %     Results.p(ivox) = p;
    %     Results.func(ivox) = func;
    %     Results.coeffweigh(ivox) = A_w;
    %     Results.pweigh(ivox) = p_w;
    %     Results.coeffweighall(ivox) = all_A_w;
    %     Results.pweighall(ivox) = all_p_w;
    %     Results.funcwall(ivox) = all_func_w;
    %
    ...
    if ismember(ivox,plot_when)
        ...
        figure; plot(theta,a.stim_RF(reorder_stim,ivox))
...
toc
save('Results_RF_only_sigma','Results','nan_vox_id');
...
% sigma = sqrt(1./all_voxel_data.kappa);
% sigma_w = sqrt(1./all_voxel_data.kappa_w);
% sigma_w_all = sqrt(1./all_voxel_data.kappa_w_all);
% 
% all_voxel_data.sigma = sigma;
% all_voxel_data.sigma_w = sigma_w;
% all_voxel_data.sigma_w_all = sigma_w_all;


% save('preference_sigma.mat','a','all_voxel_data');
